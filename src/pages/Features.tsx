    ...
    >
        ...
        <div className="max-w-6xl mx-auto">
          {features.map((feature, index) => (
            <div
              key={feature.id}
              data-index={index}
              className={`feature-section transition-all duration-1000 ${
                activeFeature === index ? 'opacity-100 scale-100' : 'opacity-0 scale-95 absolute inset-0'
              }`}
              style={{
                transform: activeFeature === index 
                  ? `
      ...
      <style jsx>{`
        ...
        @keyframes slide-in-left-3d {
          0% { 
            transform: translateX(-50px) translateZ(-30px) rotateY(-15deg); 
            opacity: 0; 
          }
          100% { 
